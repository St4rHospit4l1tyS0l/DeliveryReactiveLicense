...
using DeliveryReactiveLicensing.Models;
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (user != null)
            {
                ...
                {
                    await SendEmailConfirmationTokenAsync(user.Id, "Confirmar cuenta de correo (reenvío)");

                    // Uncomment to debug locally  
                    // ViewBag.Link = callbackUrl;
                    ViewBag.errorMessage = "Usted debe verificar su cuenta de correo antes de ingresar. "
                                         + "La liga de confirmación se ha reenviado a su cuenta de correo.";
                    //return View("Error");
                    return Json(new ResponseMsg { IsSuccess = false, Message = ViewBag.errorMessage });
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            if (!ModelState.IsValid)
            {
                return Json(new ResponseMsg { IsSuccess = false, Message = "Correo no válido, contraseña no válida o confirmación no válida" });
            }
            var user = await UserManager.FindByNameAsync(model.Email);
            if (user == null)
            {
                // Don't reveal that the user does not exist
                return Json(new ResponseMsg { IsSuccess = false, Message = "Sus datos son incorrectos, por favor intente de nuevo" });
                //return RedirectToAction("ResetPasswordConfirmation", "Account");
            }
            var result = await UserManager.ResetPasswordAsync(user.Id, model.Code, model.Password);
            if (result.Succeeded)
            {
                return Json(new ResponseMsg { IsSuccess = true, UrlToGo = Url.Action("Index", "Home") });
                //return RedirectToAction("ResetPasswordConfirmation", "Account");
            }
            AddErrors(result);
            return Json(new ResponseMsg { IsSuccess = false, Message = String.Join(", ", result.Errors.ToArray()) });
            //return View();
        }
        ...
        private async Task<string> SendEmailConfirmationTokenAsync(string userId, string subject)
        {
            ...
            if (Request.Url != null)
            {
                ...
                await UserManager.SendEmailAsync(userId, subject, "Por favor verifica tu cuenta. Debes dar clic <a href=\"" + callbackUrl + "\">aquí</a>");
                return callbackUrl;
            }
            return null;
        }
